...
// draw score function
drawText(playerPaddleRI.score, 
(1 * canvasEl.width ) / 4, 
(1 * canvasEl.height )/ 10,
"white"
);

drawText(playerPaddleAI.score, 
    (3 * canvasEl.width ) / 4 , 
    (1 * canvasEl.height )/ 10,
    "white"
    );
...
//  Creating the AI
 ...
 playerPaddleAI.yP += (ball.yP - (playerPaddleAI.yP + playerPaddleAI.height / 2)) * 
 intelligenceLevel;

 //  bouncing off the top bottom walls
 ...
 if(paddleColliDete(ball , player)) {
    ...
    // when the bal hits the paddle of any player
    ...
    // Calculating the angle at which the ball bounces back (radians)
